    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            OpendogTheme {
                ...
                var showOcrSettings by rememberSaveable { mutableStateOf(false) }
                BackHandler(enabled = showOcrSettings) {
                    showOcrSettings = false
                }
                if (showOcrSettings) {
                    OcrSettingsScreen(
                        apps = uiState.ocrApps,
                        onBack = { showOcrSettings = false },
                        onModeChanged = vm::updateOcrMode
                    )
                } else {
                    OpenDogScreen(
                        state = uiState,
                        ...
                        onOpenOcrSettings = { showOcrSettings = true },
...
private fun OpenDogScreen(
    ...
    Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
        Column(
            ...
            HorizontalDivider()
            SectionTitle("OCR")
            Text("Android 11+ uses on-device ML Kit when accessibility text is insufficient.")
            Button(onClick = onOpenOcrSettings) {
                Text("Per-app OCR settings")
            }
...
private fun OcrSettingsScreen(
    apps: List<OcrAppSetting>,
    onBack: () -> Unit,
    onModeChanged: (String, OcrMode) -> Unit
) {
    ...
    Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
        Column(
            ...
            Row(
                ...
                Text("Per-app OCR", style = MaterialTheme.typography.headlineSmall)
                Button(onClick = onBack) {
                    Text("Back")
                }
            }
            Text("AUTO uses accessibility first and falls back to OCR when text quality is low.")
            OutlinedTextField(
                value = query,
            ...
            LazyColumn(
                ...
                items(filteredApps, key = { it.packageName }) { app ->
                    Row(
                        ...
                        OcrModeSelector(
                            selectedMode = app.mode,
                            onModeSelected = { mode -> onModeChanged(app.packageName, mode) }
                        )
                    }
                    HorizontalDivider()
                }
